...
## 추가 학습 내용 정리
### JUnit4 -> JUnit5 변경 방법
(수업 따라갈 때 내가 변경해서 사용하는 방법) -> 이후 내용 추가 및 변경 필요.
- @RunWith(SpringRunner.class) 어노테이션이 @SpringBootTest 어노테이션에 포함되어 더 이상 따로 적을 필요가 없어짐.
- assertThat을 사용할 때 AssertJ에 포함된 함수를 사용.
- 오류가 발생하는 상황을 테스트할 때 @Test(expected = error 종류)를 대신하여 jupiter에 포함된 assertThrows를 사용하여 테스트 진행.
  - 메서드 정리
    - assertThat
    - assertThrows
